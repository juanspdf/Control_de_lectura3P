...
export class RabbitMQService implements OnModuleInit {
  ...
  private readonly queueOrdersResponse = process.env.RABBITMQ_QUEUE_ORDERS_RESPONSE || 'orders.response.queue';
    ...
    try {
      ...
      this.channelWrapper = this.connection.createChannel({
        ...
        setup: async (channel: Channel) => {
          ...
          await channel.assertQueue(this.queueOrdersResponse, { durable: true });
          
          await channel.bindQueue(
            this.queueOrdersResponse,
            this.exchange,
  ...
  getResponseQueue(): string {
    return this.queueOrdersResponse;
  }
}
